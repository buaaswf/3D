...
void rawarray(int xsize,int ysize,int zsize,PIXTYPE *yy)
{
	...
	try
	{
			...
			//p+=zsize;
			PIXTYPE *p=new PIXTYPE[xsize*ysize*zsize];
		for(i=0;i<xsize*ysize*zsize;i++)
		{
			...
			try
			{
				...
				Raw *rawData=new Raw(xsize,ysize,zsize,p);
...
{
	int imax, jmax,kmax, i, j,k, lev, m, n;
	...
	float sigmaC, float sigmaR, int filterSize)
{
	...
	PIXTYPE tmpX, tmpY, tmpZ,posDiff, gradDiff, domainWeight, rangeWeight, normFactor, g1, g2;
	...
	Raw* fTheta, float sigmaCTheta, float sigmaRTheta)
{
	...
	for(i=0; i<imax; i++) { //X scankline...
		for(j=0; j<jmax; j++) {	//Y scanline...
			...
			//halfSize=1.5;
			if(halfSize>3){halfSize=3;}//halfsize=5
...
void Raw3D::sizer(int ixsize, int iysize, int izsize,int rawNum) {
	...
	if(z!=NULL)
		delete[]this->z;
	z = new Raw[izsize];			// make room for the 2D objects,
	rawNum = izsize;
	for(i=0; i< rawNum; i++) 
		z[i].sizer(ixsize,iysize,izsize);	// use the Raw sizer.	
}
